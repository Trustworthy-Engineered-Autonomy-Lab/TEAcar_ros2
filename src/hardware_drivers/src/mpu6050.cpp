...
void MPU6050::setGyroscopeRange(MPU6050::GyroRange range)
{
...
void MPU6050::setAccelerometerRange(MPU6050::AccelRange range)
{
...
double MPU6050::convertRawGyroscopeData(int16_t gyro_raw) const
{
    const double ang_vel_in_deg_per_s = static_cast<double>(gyro_raw) / GYRO_SENS_MAP.at(gyro_range_);
    return ang_vel_in_deg_per_s;
}

double MPU6050::convertRawAccelerometerData(int16_t accel_raw) const
{
...
void MPU6050::setGyroscopeOffset(double gyro_x_offset, double gyro_y_offset,
...
void MPU6050::setAccelerometerOffset(double accel_x_offset, double accel_y_offset,
...
void MPU6050::reportError(int error) { std::cerr << "Error! Errno: " << strerror(error); }
